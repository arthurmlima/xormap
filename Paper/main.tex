...
\pgfplotsset{
  ...
  % Three equal panels across a figure* (plain image | cipher image |
  % correlation) -- grayscale has one channel, so one correlation panel.
  paper third plot/.style={
    width=0.30\textwidth, height=0.30\textwidth,
    grid=both,
    grid style={line width=0.1pt, draw=black!10},
    major grid style={line width=0.2pt, draw=black!25},
    tick label style={font=\scriptsize},
  ...
  % Five equal panels across a figure* (plain image | cipher image | R | G
  % | B correlation) -- RGB888 has three channels, one panel each.
  paper fifth plot/.style={
    width=0.185\textwidth, height=0.185\textwidth,
    grid=both,
    grid style={line width=0.1pt, draw=black!10},
    major grid style={line width=0.2pt, draw=black!25},
...
\addplot[paper scatter] table[x=K,y=value,col sep=comma]
    {../Tikz/data/gray_entropy_points.csv};
...
Table~\ref{tab:setup}. Fig.~\ref{fig:gray-panel} shows the grayscale
plaintext, ciphertext, and horizontal adjacent-pixel correlation;
Fig.~\ref{fig:house-panel} shows the same for \texttt{house}, with
correlation broken out per channel; Fig.~\ref{fig:histogram} shows the
corresponding pixel-value histograms. For \texttt{5.3.01}, horizontal
correlation drops from $0.979$ (plaintext) to $-0.034$ (cipher); for
\texttt{house}, from $0.952/0.935/0.969$ (R/G/B plaintext) to
$0.013/{-0.030}/{-0.027}$ (R/G/B cipher) — in every case the strong
diagonal structure visible in the plaintext scatter collapses to an
uncorrelated cloud, and the corresponding histogram flattens from the
image's natural, highly non-uniform tonal distribution toward the
near-flat cipher histogram already quantified by Table~\ref{tab:chi2} at
this $K$.

\begin{figure*}[htbp]
\centering
\includegraphics[width=0.30\textwidth]{img/gray_plain_full.png}\hfill
\includegraphics[width=0.30\textwidth]{img/gray_cipher_full.png}\hfill
\begin{tikzpicture}
\begin{axis}[paper third plot,
...
\addplot[paper corr plain] table[x=plain_x,y=plain_y,col sep=comma]
    {data/gray_correlation.csv};
\addplot[paper corr cipher] table[x=cipher_x,y=cipher_y,col sep=comma]
    {data/gray_correlation.csv};
\legend{plain,cipher}
\end{axis}
\end{tikzpicture}
\caption{Grayscale \texttt{5.3.01}, $K=384$: plaintext and ciphertext
(full-resolution $1024\times1024$ raster) and horizontal adjacent-pixel
correlation (vector, 2000 sampled pixel pairs per series).}
\label{fig:gray-panel}
\end{figure*}

\begin{figure*}[htbp]
\centering
\includegraphics[width=0.185\textwidth]{img/house_plain_full.png}\hfill
\includegraphics[width=0.185\textwidth]{img/house_cipher_full.png}\hfill
\begin{tikzpicture}
\begin{axis}[paper fifth plot, title={R correlation},
    xmin=0, xmax=255, ymin=0, ymax=255]
\addplot[paper corr plain] table[x=plain_x,y=plain_y,col sep=comma]
    {data/house_correlation_r.csv};
\addplot[paper corr cipher] table[x=cipher_x,y=cipher_y,col sep=comma]
    {data/house_correlation_r.csv};
\end{axis}
\end{tikzpicture}\hfill
\begin{tikzpicture}
\begin{axis}[paper fifth plot, title={G correlation},
    xmin=0, xmax=255, ymin=0, ymax=255]
\addplot[paper corr plain] table[x=plain_x,y=plain_y,col sep=comma]
    {data/house_correlation_g.csv};
\addplot[paper corr cipher] table[x=cipher_x,y=cipher_y,col sep=comma]
    {data/house_correlation_g.csv};
\end{axis}
\end{tikzpicture}\hfill
\begin{tikzpicture}
\begin{axis}[paper fifth plot, title={B correlation},
    xmin=0, xmax=255, ymin=0, ymax=255]
\addplot[paper corr plain] table[x=plain_x,y=plain_y,col sep=comma]
    {data/house_correlation_b.csv};
\addplot[paper corr cipher] table[x=cipher_x,y=cipher_y,col sep=comma]
    {data/house_correlation_b.csv};
\end{axis}
\end{tikzpicture}
\caption{RGB888 \texttt{house}, $K=384$: plaintext and ciphertext
(full-resolution $512\times512$ raster) and horizontal adjacent-pixel
correlation per channel (vector, 2000 sampled pixel pairs per series;
orange = plaintext, blue = cipher, as in Fig.~\ref{fig:gray-panel}).}
\label{fig:house-panel}
\end{figure*}
