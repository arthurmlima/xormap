...
% --- parallel pool ---
% Size the pool to this machine's logical core count (nproc) rather than
% to a fixed 24 or to feature('numcores') (physical cores only, 12 on this
% host vs 24 logical): the (image, K) task split works on any core count,
% fewer workers just means proportionally more wall-clock time.
num_workers = min(local_nproc(), num_tasks);
pool = gcp('nocreate');
...
saveas(fh, png_path);
pdf_path = fullfile(results_dir, 'sweep_k_xorfold_rgb888.pdf');
