function analysis_xorfold_rgb888()
%ANALYSIS_XORFOLD_RGB888 Key sensitivity, histogram and PSNR analysis, RGB888 XOR-fold.
%   The XOR-fold counterpart of ../xormap_image_rgb888_matlab/analysis_rgb888.m
%   (itself the colour counterpart of ../xormap_image_matlab/analysis_gray.m):
%   the same three tests, run against xormap_rgb888_xorfold_encrypt/decrypt
%   (one folded K-bit state per pixel, see README.md) across every SIPI
%   colour image (51, see ../xormap_image_rgb888_matlab/download_images.m)
%   at K = 24:24:384 -- the same grid as sweep_k_xorfold_rgb888.m. Writes
%   one CSV and one vector PDF per test into results/:
%
%     key_sensitivity_xorfold_rgb888.{csv,pdf}  NPCR/UACI/PSNR between two
%                                      ciphers whose keys differ in ONE
%                                      bit, the PSNR of a wrong-key
%                                      decryption, and a per-bit study.
%     histogram_analysis_xorfold_rgb888.{csv,pdf}  chi-square uniformity
%                                      of the R, G and B histograms.
...
%   ONE pass rather than three -- the same cost-sharing argument as the
%   non-fold sibling, but more valuable here: every xormap_rgb888_xorfold_
%   encrypt call performs N state transitions REGARDLESS of K, so this is
%   already the most expensive analysis script in the project (see
%   ../xormap_image_rgb888_xorfold_matlab/README.md's "1.1 billion state
%   transitions" note on the sweep this mirrors).
%
%   WHAT IS MEASURED ON WHAT. Chi-square and PSNR are computed on the
%   three standard 8-bit channels from rgb888_unpack, so the ideals are
%   the ordinary 8-bit ones (dof=255, peak=255) -- unlike the RGB565
%   sibling, RGB888's channels are all uniformly 8 bits, so no per-channel
%   critical value or peak adjustment is needed. NPCR/UACI are computed on
%   the packed 24-bit word (L=2^24), matching sweep_k_xorfold_rgb888.m, so
%   their ideals come from npcr_uaci_ideal(24) -- NOT the 8-bit values.
%
...
%   round-trip PSNR is exactly +Inf by construction (lossless XOR cipher,
%   MSE identically zero) -- asserted as a correctness check, not a tuning
%   signal.
%
...
%
%   which again depends only on key1 XOR key2, not on the plaintext.
%   Tested directly below (the difference is checksummed in row-major
%   pixel order) rather than assumed.
    ...
    if ~exist(manifest_path, 'file')
        error('analysis_xorfold_rgb888:noManifest', ...
            'Manifest not found: %s. Run ../xormap_image_rgb888_matlab/download_images.m first.', manifest_path);
    end
    [files, names] = read_manifest(manifest_path, img_dir);
    ...
    parfor idx = 1:num_tasks
        ...
        [c1, seed1] = xormap_rgb888_xorfold_encrypt(plain, key1);
        c2          = xormap_rgb888_xorfold_encrypt(plain, key2);

        cipher_diff = bitxor(c1, c2);

        % Row-major: matches the pixel-major keystream layout the
        % encrypt/decrypt functions use (they reshape via .' too).
        ...
        if idx == 1
            [~, seed2] = xormap_rgb888_xorfold_encrypt(plain, key2);
            assert(isequal(xormap_rgb888_xorfold_decrypt(c1, seed2), wrong), ...
                'wrong-key shortcut disagrees with decrypt()');
        end

        recovered = xormap_rgb888_xorfold_decrypt(c1, seed1);
    ...
    % Smallest image (65536 px, misc volume) for the per-bit study: every
    % xorfold encryption is O(N) regardless of K, so this is chosen
    % explicitly for wall-clock, not taken as files{1} by accident.
    bit_study_idx = pick_smallest_image(files);
    B = bit_position_study(files{bit_study_idx}, K_VALUES);

    write_key_sensitivity(results_dir, R, B, names, task_img, K_VALUES, ...
        FLIP_BIT, same_per_K);
    write_histogram(results_dir, R, names, task_img, K_VALUES, files);
    write_psnr(results_dir, R, names, task_img, K_VALUES);
end

% ---------------------------------------------------------------------------

function idx = pick_smallest_image(files)
    n = numel(files);
    px = zeros(1, n);
    for i = 1:n
        info = imfinfo(files{i});
        px(i) = info(1).Height * info(1).Width;
    end
    [~, idx] = min(px);
end

function B = bit_position_study(fname, K_VALUES)
% Rows: [K, bit, npcr, uaci, psnr]. One (small) image suffices given the
% image-independence result checked in the main pass.
%
% Sampled at up to 12 positions per K, not the 24 used by the non-fold
% sibling: every xormap_rgb888_xorfold_encrypt call performs N state
% transitions regardless of K, so halving the sample count roughly halves
% this loop's wall-clock without materially changing what the spread shows.
    ...
    parfor ki = 1:numel(K_VALUES)
        ...
        key1 = secret_key(K);
        c1 = xormap_rgb888_xorfold_encrypt(plain, key1);
...
% which MUST already be in row-major (pixel/keystream) order.
    d = double(diff_row(:));
    n = min(numel(d), 4096);
...
function write_key_sensitivity(results_dir, R, B, names, task_img, ...
                               K_VALUES, flip_bit, same_per_K)
    csv = fullfile(results_dir, 'key_sensitivity_xorfold_rgb888.csv');
    fid = fopen(csv, 'w');
    ...
    for r = 1:size(R, 1)
        fprintf(fid, '%s,%d,%d,%d,%.6f,%.6f,%.4f,%.4f,%d,%d\n', ...
            names{task_img(r)}, R(r,1), R(r,2), flip_bit, ...
            R(r,3), R(r,4), R(r,5), R(r,6), R(r,13), R(r,12));
    end
    fclose(fid);
    fprintf('Wrote %s\n', csv);

    csv_b = fullfile(results_dir, 'key_sensitivity_bits_xorfold_rgb888.csv');
    fid = fopen(csv_b, 'w');
    ...
    title(ax, 'By flipped bit position (single, smallest image)', ...
    ...
    layout_title(tl, {'Key sensitivity: RGB888 XOR-fold, every SIPI colour image', ...
        sprintf(['one flipped key bit \\bullet NPCR/UACI on the packed 24-bit word ' ...
                 '\\bullet difference image-independent: %s'], verdict)});
    export_pdf(fh, fullfile(results_dir, 'key_sensitivity_xorfold_rgb888.pdf'));
end

function write_histogram(results_dir, R, names, task_img, K_VALUES, files)
    csv = fullfile(results_dir, 'histogram_analysis_xorfold_rgb888.csv');
    fid = fopen(csv, 'w');
    fprintf(fid, ['image,K,num_pixels,mean_chi2_plain,mean_chi2_cipher,' ...
        'chi2_critical_005,cipher_uniform_pass\n']);
    for r = 1:size(R, 1)
        fprintf(fid, '%s,%d,%d,%.4f,%.4f,%.4f,%d\n', ...
            names{task_img(r)}, R(r,1), R(r,2), R(r,7), R(r,8), R(r,9), R(r,8) <= R(r,9));
    end
    fclose(fid);
    fprintf('Wrote %s\n', csv);
    ...
    K = K_VALUES(end);
    cipher = xormap_rgb888_xorfold_encrypt(plain, secret_key(K));
    ...
    layout_title(tl, {'Histogram analysis: RGB888 XOR-fold, every SIPI colour image', ...
        sprintf(['\\chi^2 uniformity per 8-bit channel \\bullet cipher passes at ' ...
                 '\\alpha=0.05 in %.1f%% of (image, K) cases'], pass_rate)});
    export_pdf(fh, fullfile(results_dir, 'histogram_analysis_xorfold_rgb888.pdf'));
end

function write_psnr(results_dir, R, names, task_img, K_VALUES)
    csv = fullfile(results_dir, 'psnr_analysis_xorfold_rgb888.csv');
    fid = fopen(csv, 'w');
    fprintf(fid, ['image,K,num_pixels,psnr_plain_cipher_db,' ...
        'psnr_plain_wrongkey_db,psnr_roundtrip_db\n']);
    for r = 1:size(R, 1)
        fprintf(fid, '%s,%d,%d,%.4f,%.4f,%s\n', ...
            names{task_img(r)}, R(r,1), R(r,2), R(r,10), R(r,6), fmt_inf(R(r,11)));
    end
    fclose(fid);
    fprintf('Wrote %s\n', csv);
    ...
    layout_title(tl, {'PSNR: RGB888 XOR-fold, every SIPI colour image', ...
        sprintf(['peak 255 over R/G/B \\bullet round-trip PSNR is +Inf in %d/%d ' ...
                 'cases (XOR is lossless)'], n_inf, size(R,1))});
    export_pdf(fh, fullfile(results_dir, 'psnr_analysis_xorfold_rgb888.pdf'));
end
